...
class ControllerExtensionModulePavcustom extends Controller {
	...
	public function index() {  
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('extension/module/pavcustom', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if( isset($this->request->get['module_id'])  && $action == 'saveedit' ) {
				$this->response->redirect($this->url->link('extension/module/pavcustom', 'module_id='.$this->request->get['module_id'].'&token=' . $this->session->data['token'], 'SSL'));
			}else{
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			$this->mdata['action'] = $this->url->link('extension/module/pavcustom', 'module_id='.$this->request->get['module_id'].'&token=' . $this->session->data['token'], 'SSL');
		}else {
			$module_info = $default;
			$this->mdata['selectedid'] = 0;
			$this->mdata['subheading'] = $this->language->get('text_create_new_module');
			$this->mdata['action'] = $this->url->link('extension/module/pavcustom', 'token=' . $this->session->data['token'], 'SSL');
		}
		...
		$moduletabs = $this->model_extension_module->getModulesByCode( 'pavcustom' );
	 	$this->mdata['link'] = $this->url->link('extension/module/pavcustom', 'token=' . $this->session->data['token'] . '', 'SSL');
		...
		foreach( $this->mdata['languages'] as $lang ){
			if( empty( $module_info['description'][$lang['language_id']] ) ) {
				$module_info['description'][$lang['language_id']] = '';
			}
			if( empty($module_info['module_title'][$lang['language_id']]) ){
		...
		$this->response->setOutput($this->load->view('extension/module/pavcustom.tpl', $this->mdata));
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/pavcustom')) {
